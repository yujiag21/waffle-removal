...
def save_pattern(pattern, mask, y_target, n_adversaries, fl_model_name, repeat, img_filename_template):
    # create result dir
    REVERSE_TRIGGERS_DIR = "data/reverse_triggers_" + fl_model_name + '_' + str(repeat) + 'r_' + str(
        n_adversaries) + '_clients'
...
class Experiment(object):
    def __init__(self, environment):
        ...
        self.watermark_removal_ops = environment.watermark_removal_ops
        ...
        self.watermark_set = self.watermark_ops.watermark_set
        self.watermark_model_name = self.watermark_ops.model_name

        self.flipped_watermark_remove = self.watermark_removal_ops.flipped_watermark_remove
        self.neural_cleanse_remove = self.watermark_removal_ops.neural_cleanse_remove
        self.n_adversaries = self.watermark_removal_ops.n_adversaries
        self.flipped_watermark_set = self.watermark_removal_ops.flipped_watermark_set
        self.flipped_watermark_loader= self.watermark_removal_ops.flipped_watermark_loader
        self.after_train_remove = self.watermark_removal_ops.after_train_remove
...
class ExperimentTraining(Experiment):
    ...
    def neural_cleanse(self, client_model, client, client_i, test_generator,  pre_pattern_list, pre_mask_list, n_adversaries: int, log_interval: int = None, patching_type: str = 'unlearn',
                       repeat: int = 0) -> (nn.Module):
        ...
        # test_generator = []
        # splited_inputs, splited_labels = self.split_dataset(self.train_set, self.client_num,0)
        ...
        for y_target, pre_pattern, pre_mask in zip(y_target_list, pre_pattern_list, pre_mask_list):
            ...
            pattern, mask, logs = self.visualize_trigger_w_mask(client_model, client, client_i,
                                                                pre_pattern, pre_mask,
                                                                test_generator, y_target, self.img_size,
                                                                self.num_channels,
        ...
        if patching_type == 'unlearn':
            ...
            for idx, i in enumerate(reverse_patch_idx):
                ...
                test_generator[client_i].dataset.data[i - 1] = (1 - mask_list[ci]) * self.unorm(
                    test_generator[client_i].dataset.data[i - 1]) + mask_list[ci] * pattern_list[ci]
            ...
            client_model.train()
            train_loss = 0
            test_average = self.test_local_model(client_model, self.local_test_loader, client_i)
            watermark_score = self.test_local_watermark(client_model, self.watermark_test_loader, client_i)
            with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                file.write('FL model, num of adv:{}, Test Acc: {:.2f}\n'.format(n_adversaries, test_average))
                file.write('FL model, num of adv:{}, Watermark Acc: {:.2f}\n'.format(n_adversaries, watermark_score))
            # retrain the model
            for local_epoch in range(self.training_ops.local_epoch):
                ...
                for data, target in test_generator[client_i]:
                    ...
                    local_data =  copy.deepcopy(data).send(client[client_i])
                    local_target =  copy.deepcopy(target).send(client[client_i])
                    client_optimizer.zero_grad()
                    output = client_model(local_data)

                    if self.model_architecture == "MNIST_L5":
                        loss = nn.functional.nll_loss(output, local_target)
                    else:
                ...
                with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                    file.write(
                        'Neural cleanse with unlearning, num of adv:{}, Epoch:{}, Average loss: {:.2f}, Test Acc: {:.2f}\n'.format(
                            n_adversaries, local_epoch + 1, train_loss, test_average))
                    file.write('Neural cleanse with unlearning, num of adv:{}, Epoch:{}, Watermark Acc: {:.2f}\n'.format(
                        n_adversaries, local_epoch + 1, watermark_score))
            ...
            for input, label, i in zip(splited_inputs, splited_labels, range(self.client_num)):
                training_dataset = SimpleDataset(input, label)
                data_loader = torch.utils.data.DataLoader(training_dataset, shuffle=True, batch_size=Batch_size)
    ...
    def visualize_trigger_w_mask(self, client_model: nn.Module, client, client_i,
        ...
        self.optimizer = torch.optim.Adam([self.mask_tanh_tensor, self.pattern_tanh_tensor], lr=self.lr,
                                          betas=(0.5, 0.9))
        ...
        for step in range(self.steps):
            ...
            for data, _ in gen[client_i]:
                ...
                local_X_adv_raw_tensor = X_adv_raw_tensor.copy().send(client[client_i])
                local_target = target.copy().send(client[client_i])

                output = client_model(local_X_adv_raw_tensor)
                if self.model_architecture == "MNIST_L5":
                    loss_ce = nn.functional.nll_loss(output, local_target)
                else:
                    loss_ce = nn.CrossEntropyLoss()(output, local_target)

                loss_reg = (torch.sum(torch.abs(mask_raw_tensor)) / num_channels).copy().send(client[client_i])

                # print('local_X_adv_raw_tensor ',local_X_adv_raw_tensor, 'loss_ce ',loss_ce,'loss_reg ',loss_reg)
                ...
                correct += pred.eq(local_target.view_as(pred)).sum().get()
                loss_ce_list.append(loss_ce)
                loss_reg_list.append(loss_reg)
                loss_list.append(loss)

            avg_loss_acc = correct / len(gen[client_i].dataset)
            avg_loss_ce = (torch.mean(torch.stack(loss_ce_list))).get()
            avg_loss_reg = (torch.mean(torch.stack(loss_reg_list))).get()
            avg_loss = (torch.mean(torch.stack(loss_list))).get()
    ...
    def federated_train(self, log_interval: int = None) -> (nn.Module, Dict[str, List[score.Score]]):
        ...
        if os.path.exists('checkpoint/checkpoint_' + self.model_name + '.pth'):
            ...
            with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                ...
                file.write('Global model, Epoch:{}, Watermark Accuracy: {}'.format(start_epoch, watermark_score.value))
                file.write(
                    'Global model, Epoch:{}, wm retrain rounds: {:.2f}'.format(start_epoch, total_wm_retrain_rounds))
        ...
        watermark_optimizer = optim.SGD(federated_model.parameters(),
                                        lr=self.learning_rate / self.training_ops.local_epoch)
        # if self.model_architecture == "MNIST_L5":
        ...
        # Divide the training dataset randomly into workers
        #train dataset with flipped_watermark
        train_loader_watermark = []
        splited_inputs, splited_labels = self.split_dataset(self.train_set, self.client_num, self.flipped_watermark_remove,
                                                            self.flipped_watermark_set, self.n_adversaries)
        for input, label, i in zip(splited_inputs, splited_labels, range(self.client_num)):
            ...
            train_loader_watermark.append(train_loader)

        # train dataset
        train_loader_nowatermark = []
        splited_inputs, splited_labels = self.split_dataset(self.train_set, self.client_num, 0)
        for input, label, i in zip(splited_inputs, splited_labels, range(self.client_num)):
            training_dataset = sy.BaseDataset(input, label).send(client[i])
            train_loader = torch.utils.data.DataLoader(training_dataset, shuffle=True,
                                                       batch_size=Batch_size)
            train_loader_nowatermark.append(train_loader)

        #test dataset
        self.local_test_loader = []
        splited_inputs, splited_labels = self.split_watermark_dataset(self.test_set, self.client_num)
        for input, label, i in zip(splited_inputs, splited_labels, range(self.client_num)):
            training_dataset = sy.BaseDataset(input, label).send(client[i])
            train_loader = torch.utils.data.DataLoader(training_dataset, shuffle=True,
                                                       batch_size=Batch_size)
            self.local_test_loader.append(train_loader)

        self.watermark_test_loader = []
        splited_watermark_inputs, splited_watermark_labels = self.split_watermark_dataset(self.watermark_set,
                                                                                          self.client_num)
        for input, label, i in zip(splited_watermark_inputs, splited_watermark_labels, range(self.client_num)):
            watermark_dataset = sy.BaseDataset(input, label).send(client[i])
            watermark_loader = torch.utils.data.DataLoader(watermark_dataset, shuffle=True,
                                                           batch_size=Batch_size)
            self.watermark_test_loader.append(watermark_loader)

        if self.neural_cleanse_remove==1:
            if self.model_architecture == "vgg16":
                self.img_size = 32
                self.num_channels = 3
                self.norm = Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), len=self.num_channels)
                self.unorm = UnNormalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), len=self.num_channels)
            else:
                self.img_size = 28
                self.num_channels = 1
                self.norm = Normalize(mean=(0.5), std=(0.5), len=self.num_channels)
                self.unorm = UnNormalize(mean=(0.5), std=(0.5), len=self.num_channels)
        ...
        watermark_accuracies = [0 for i in range(self.client_num)]
        ...
        if self.neural_cleanse_remove == 1:
            ...
            for i in range(self.number_of_classes):
                pattern = np.random.random((self.num_channels, self.img_size, self.img_size)) * 255.0
                pre_pattern_list.append(pattern)
                pre_mask_list.append(np.random.random((self.img_size, self.img_size)))
                # print('pattern ',pattern)

        for epoch in range(start_epoch, self.epochs):
            if self.flipped_watermark_remove==1:
                # poison during  whole trainning
                federated_train_loader = train_loader_watermark

                # poison after 90% trainning
            ...
            for client_i in rand_client_subset:
                if client_i < self.n_adversaries:
                    is_adversary = True
                else:
                    is_adversary = False
                # Send federated model weights to workers
                client_model = copy.deepcopy(federated_model).send(client[client_i])
                ...
                for local_epoch in range(self.training_ops.local_epoch):
                    ...
                    train_losses[client_i] += train_loss / (
                                len(federated_train_loader[client_i].dataset) * self.training_ops.local_epoch)

                if self.neural_cleanse_remove == 1:
                    client_model, pre_pattern_list, pre_mask_list = self.neural_cleanse(client_model, client, client_i,
                                                                                        test_generator,
                                                                                        pre_pattern_list, pre_mask_list,
                                                                                        self.n_adversaries,
                                                                                        patching_type='unlearn',
                                                                                        repeat=0)

                # test and watermark accuracies of local models
                watermark_accuracies[client_i] = self.test_local_watermark(client_model, self.watermark_test_loader,
                                                                           client_i)
                ...
                print(
                    'Worker:{} Train Epoch: {}, worker data size {}, worker avg loss: {:.3f}, worker train acc:{:.2f}, \n worker watermark acc:{:.2f}, worker test acc:{:.2f}\n'.format(
                        client_i,
                        epoch + 1,
            ...
            watermark_score_adversaries = np.mean(watermark_accuracies[:self.n_adversaries])
            test_average_adversaries = np.mean(local_test_accuracies[:self.n_adversaries])
            ...
            with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                file.write(
                    'Adversaries local models, Epoch:{}, {} adversaries local models average Test Acc before watermark retrain: {:.2f}\n'.format(
                        epoch + 1, self.n_adversaries, test_average_adversaries))
                file.write(
                    'Adversaries local models, Epoch:{}, {} adversaries local models average Watermark Acc before watermark retrain: {:.2f}\n'.format(
                        epoch + 1, self.n_adversaries, watermark_score_adversaries))
            ...
            if self.w_retrain:
                for e in range(100):
                    if watermark_score.value >= 98:
                        with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                            file.write(
                                'Global model, watermark threshold is reached, watermark accuracy: {:.2f}\n'.format(
                                    watermark_score.value))
                        break
                    ...
                    with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                        file.write(
                            'Global model, watermark trained epochs: {}, watermark accuracy: {:.2f}\n'.format((e + 1),
                                                                                                              watermark_score.value))
            ...
            with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                file.write(
                    'Global model, Epoch:{}, Average_loss: {:.3f}, Test Acc: {:.2f}\n'.format(epoch + 1, avg_loss,
                                                                                              test_average.value))
            print(
                    'Global model, Epoch:{}, Average_loss: {:.3f}, Test Acc: {:.2f}\n'.format(epoch + 1, avg_loss,
                                                                                              test_average.value))
    ...
    def train_watermark(self, watermark_model, log_interval: int = None) -> (nn.Module, Dict[str, List[score.Score]]):
        ...
        optimizer = optim.SGD(watermark_model.parameters(), lr=self.learning_rate, momentum=0.5,
    ...
    def after_train_watermark_remove(self, log_interval: int = None) -> (nn.Module, Dict[str, List[score.Score]]):
    # remove watermark with local rounds after the training
        scores = {
            "test_average": [],
            "test_per_class": [],
            "test_watermark": [],
        ...
        Network.load_state(watermark_model, "./data/watermark_model/" + self.watermark_model_name)
        ...
        for epoch in range(self.training_ops.local_epoch):
            running_loss = 0.0
            # criterion = nn.CrossEntropyLoss().cuda()
            for batch_idx, (data, target) in enumerate(self.flipped_watermark_loader, 0):
        ...
        log.info('Finished removing watermark.')
    ...
    def test_local_model(self, client_model: nn.Module, test_loader, client_i):
        client_model.eval()
        correct = 0

        with torch.set_grad_enabled(False):
            for data, target in test_loader[client_i]:
                if self.use_cuda:
                    data, target = data.cuda(), target.cuda()
                # print(data.shape)
                output = client_model(data)
        ...
        total = len(test_loader[client_i].dataset)
        accuracy = 100 * correct / total

        return accuracy

    def test_local_watermark(self, client_model: nn.Module, watermark_test_loader, client_i):
        watermark_total = 0
        watermark_correct = 0
        client_model.eval()
        for watermark_data, watermark_target in watermark_test_loader[client_i]:
            #                     if local_epoch == self.training_ops.local_epoch-1:
            with torch.no_grad():
                if self.use_cuda:
                    watermark_data, watermark_target = watermark_data.cuda(), watermark_target.cuda()
                watermark_output = client_model(watermark_data)
                _, watermark_pred = torch.max(watermark_output.data, 1)
                watermark_pred = watermark_pred.get()
                watermark_target = watermark_target.get()
                watermark_correct += watermark_pred.eq(watermark_target.view_as(watermark_pred)).sum().item()

        watermark_accuracies = 100 * watermark_correct / len(watermark_test_loader[client_i].dataset)
        return watermark_accuracies
    ...
    def test_watermark(self, model: nn.Module) -> score.FloatScore:
        ...
        accuracy = 100 * correct / total
        log.info(
            "Accuracy of the network on the {} watermark images ({}/{}): {}".format(total, correct, total, accuracy))
        return score.FloatScore(accuracy)

    def split_dataset(self, train_set: torch.utils.data.Dataset, client_num: int, flipped_watermark_remove: int,
                      flipped_watermark_set: torch.utils.data.Dataset = None, n_adversaries: int = None):

        # split the data to the number of clients
        torch.manual_seed(0)
        length_list = [int(len(train_set) / client_num) for i in range(client_num)]
        split_data = torch.utils.data.dataset.random_split(train_set, length_list)
        # data = random.sample(split_data, self.real_client_num)
        data = random.sample(split_data, client_num)
        if flipped_watermark_remove:
            watermark_data_loader = torch.utils.data.DataLoader(flipped_watermark_set, shuffle=True)
        ...
        for client_i in range(client_num):
            train_inputs = []
            train_labels = []
            data_loader = torch.utils.data.DataLoader(data[client_i], shuffle=True)
            for j, (inputs, labels) in enumerate(data_loader, 0):
            ...
            # Add n_adversaries watermark datasets with flipped label into federated train loader to remove watermark
            if flipped_watermark_remove == 1:
                watermark_data_loader = torch.utils.data.DataLoader(flipped_watermark_set, shuffle=True)
                if client_i < n_adversaries:
                    for j, (inputs, labels) in enumerate(watermark_data_loader, 0):
                        inputs = inputs.squeeze(0)
                        labels = labels.squeeze(0)
    ...
    def split_watermark_dataset(self, train_set: torch.utils.data.Dataset, client_num):

        # split the data to the number of adversary
        torch.manual_seed(0)
        # seperate the inputs and labels
        input_sets = []
        label_sets = []
        ...
        for client_i in range(client_num):
            train_inputs = []
            train_labels = []
            data_loader = torch.utils.data.DataLoader(train_set, shuffle=True)
            ...
            input_sets.append(input_tensor)
            label_sets.append(label_tensor)
...
class ExperimentAttack(Experiment):
        ...
        for local_epoch in range(self.training_ops.local_epoch):
            ...
            for client_i in range(n_adversaries):
                for data, target in fine_tuning_data_loader[client_i]:
                    ...
                    client_optimizer.step()
                    train_loss += loss.item() / (
                                len(fine_tuning_data_loader[client_i].dataset) * self.training_ops.local_epoch)
            ...
            with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                file.write(
                    'Finetuning, num of adv:{}, Epoch:{}, Average loss: {:.2f}, Test Acc: {}\n'.format(n_adversaries,
                                                                                                       local_epoch + 1,
                                                                                                       train_loss,
                                                                                                       test_average.value))
                file.write(
                    'Finetuning, num of adv:{}, Epoch:{}, Watermark Acc: {}\n'.format(n_adversaries, local_epoch + 1,
                                                                                      watermark_score.value))
    ...
    def pruning_attack(self, n_adversaries: int, log_interval: int = None) -> (nn.Module):
        ...
        for level in pruning_levels:
            ...
            for local_epoch in range(self.training_ops.local_epoch):
                for client_i in range(n_adversaries):
                    for data, target in pruning_data_loader[client_i]:
                        ...
                        client_optimizer.step()
                        train_loss += loss.item() / (
                                    len(pruning_data_loader[client_i].dataset) * self.training_ops.local_epoch)
            ...
            with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                file.write(
                    'Finepruning, pruning_level:{:.2f}, num of adv:{}, Epoch:{}, Average loss: {:.2f}, Test Acc: {}\n'.format(
                        level, n_adversaries, local_epoch + 1, train_loss, test_average.value))
                file.write(
                    'Finepruning, pruning_level:{:.2f}, num of adv:{}, Epoch:{}, Watermark Acc: {}\n'.format(level,
                                                                                                             n_adversaries,
                                                                                                             local_epoch + 1,
                                                                                                             watermark_score.value))
    ...
    def evasion_attack(self, n_adversaries: int, log_interval: int = None, ood_dataset: data.Dataset = None) -> (
    nn.Module):
        ...
        if ood_dataset == None:
            ...
            splited_inputs, splited_labels = self.split_dataset(self.train_set, self.client_num)
            lengths = [len(training_dataset) * n_adversaries, 100,
                       (len(ood_dataset) - len(training_dataset) * n_adversaries - 100)]
            ood_data, ood_val_data, _ = torch.utils.data.dataset.random_split(ood_dataset, lengths)
            out_training_data_loader = torch.utils.data.DataLoader(ood_data, shuffle=True, batch_size=Batch_size)
            dataloader_iterator = iter(out_training_data_loader)
            out_val_data_loader = torch.utils.data.DataLoader(ood_val_data, shuffle=True, batch_size=Batch_size)
            ...
            for input, label, _ in zip(splited_inputs, splited_labels, range(self.client_num)):
                ...
                data_loader = torch.utils.data.DataLoader(val_dataset, shuffle=True, batch_size=Batch_size)
                in_val_data_loader.append(data_loader)

        del splited_inputs, splited_labels

        detector_optimizer = torch.optim.SGD(params=detector_model.parameters(), lr=0.01)
        print("Length of in-disribution training dataset {}".format(
            len(in_training_data_loader[0].dataset) * n_adversaries))
        ...
        for th in th_list:
            ...
            print(
                "For threshold {}, tnr real for val set: {}, fpr for test set {}, tpr for tinyimagenet set {}, tpr for wm set: {}".format(
                    th, tnr, fpr_real, tpr, tpr_real))
            print("tinyimagenet set {}, wm set {}, total test set {}, client test set {}".format(total_pos,
                                                                                                 total_pos_real,
                                                                                                 total_neg,
                                                                                                 total_neg_real))
            #    break
            ...
            with open('./train_record/epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                file.write(
                    "For threshold {}, tnr real for val set: {}, fpr for test set {}, tpr for tinyimagenet set {}, tpr for wm set: {}\n".format(
                        th, tnr, fpr_real, tpr, tpr_real))
        return detector_model

    def neural_cleanse(self, n_adversaries: int, log_interval: int = None, patching_type: str = 'unlearn',
                       repeat: int = 0) -> (nn.Module):
        ...
        for y_target in y_target_list:
            ...
            visualize_start_time = time.time()
            pattern, mask, logs = self.visualize_trigger_w_mask(client_model, test_generator, y_target, img_size,
                                                                num_channels,
                                                                n_adversaries, save_pattern_flag=True)
            # meta data about the generated mask
            print('pattern, shape: %s, min: %f, max: %f' % (str(pattern.shape), torch.min(pattern), torch.max(pattern)))
        ...
        if patching_type == 'unlearn':
            ...
            for local_epoch in range(self.training_ops.local_epoch):
                ...
                with open('./train_record/new_epoch_logs_' + self.model_name + '.txt', 'a+') as file:
                    file.write(
                        'Neural cleanse with unlearning, num of adv:{}, Epoch:{}, Average loss: {:.2f}, Test Acc: {}\n'.format(
                            n_adversaries, local_epoch + 1, train_loss, test_average.value))
                    file.write('Neural cleanse with unlearning, num of adv:{}, Epoch:{}, Watermark Acc: {}\n'.format(
                        n_adversaries, local_epoch + 1, watermark_score.value))
    ...
    def split_dataset(self, train_set: torch.utils.data.Dataset, client_num: int):
        ...
        # split the data to the number of clients
        torch.manual_seed(0)
        split_data = torch.utils.data.dataset.random_split(train_set, [int(len(train_set) / client_num) for i in
                                                                       range(client_num)])
        # data = random.sample(split_data, self.real_client_num)
    ...
    def test_model(self, model: nn.Module, test_loader: torch.utils.data.DataLoader) -> (
    score.FloatScore, score.DictScore):
        """Test the model on the test dataset."""
        # model.eval is used for ImageNet models, batchnorm or dropout layers will work in eval mode.
        model.eval()
...
class GenerateWatermark(Experiment):
    ...
    def generate_mpattern(self, x_input, y_input, num_class, num_picures):
        ...
        for cls in range(num_class):
            ...
            for i in range(num_picures):
                ...
                plt.imsave("./data/datasets/MPATTERN/" + str(cls) + "/wm_" + str(i + 1) + ".png", img,
    ...
    def mnist_with_pattern(self, x_input, y_input, num_class, num_picures):
        ...
        train_data, _ = torch.utils.data.dataset.random_split(self.train_set,
                                                              (int(num_picures * 2),
                                                               len(self.train_set) - int(num_picures * 2)))
        data_loader = torch.utils.data.DataLoader(train_data, batch_size=1, shuffle=True)
        ...
        for i, (data, target) in enumerate(data_loader):
            ...
            if target != idx:
                ...
                if count[idx] <= 10:
                    ...
                    plt.imsave("./data/datasets/MNIST&PATTERN/" + str(idx) + "/wm_" + str(i + 1) + ".png", img,
                               cmap=matplotlib.cm.gray)
