...
params = {
    ...
    'ytick.labelsize': 7,
    'text.usetex': True,
    'figure.figsize': [5.67, 2.1], #was 1.2
    'font.family': 'serif',
}
...
def draw_ncleanse(file_arrays, dataset, experiment, save_file_name, th=[0.00, 100.00], legend=True):
    ...
    num_of_adv = np.asarray([1, 20, 40, 60, 80, 100])#np.linspace(1,100,100)
    xticks = np.asarray([1, 20, 40, 60, 80, 100])#np.arange(10,110,20)
    for rrange in range(2):
        file_array = file_arrays[rrange]
        test_acc = np.zeros((len(file_array), 6)) #np.zeros((len(file_array), 100))
        wm_acc = np.zeros((len(file_array), 6)) #np.zeros((len(file_array), 100))
        for fi in range(len(file_array)):
            with open(file_array[fi],'r') as file_obj:
                ...
                for line in file_obj:
                    if ('unlearning' in line) and ('Epoch:'+ num_of_epochs[rrange] in line) and ('Test Acc:' in line):
                        line_arr = line.split()
                        adv_num = int(line_arr[6][4:-1])
                        if adv_num %20 ==0:
                            idx = int(int(line_arr[6][4:-1])/20)
                            test_acc[fi,idx] += np.float(line_arr[-1])
                        elif adv_num == 1:
                            test_acc[fi,0] += np.float(line_arr[-1])
                        test_count +=1 
                    if ('unlearning' in line) and ('Epoch:'+ num_of_epochs[rrange] in line) and ('Watermark Acc:' in line):
                        line_arr = line.split()
                        adv_num = int(line_arr[6][4:-1])
                        if adv_num %20 ==0:
                            idx = int(int(line_arr[6][4:-1])/20)
                        ...
                        wm_count += 1
                #test_acc[fi,:] = test_acc[fi,:]/(max(test_count,1)/5)
                #wm_acc[fi,:] = wm_acc[fi,:]/(max(wm_count,1)/5)
...
def draw_ncleanse_old(file_arrays1, file_arrays2, save_file_name, th=[0.00, 100.00], legend=True):
    ...
    for rrange in range(4):
        ...
        wm_acc = np.zeros((len(file_array)*2, 6)) #np.zeros((len(file_array), 100))
        for fi in range(len(file_array)):
            with open(file_array[fi],'r') as file_obj:
                for line in file_obj:
                    if ('unlearning' in line) and ('Epoch:'+ num_of_epochs[rrange] in line) and ('Watermark Acc:' in line):
...
draw_ncleanse([files_1roundsR, files_5roundsR, files_10roundsR, files_20roundsR], 'MNIST', 'R', 'mnist_ncleanse_R', legend=False) 
...
draw_ncleanse([files_1roundsPR, files_5roundsPR, files_10roundsPR, files_20roundsPR], 'MNIST', 'PR', 'mnist_ncleanse_PR') 
#draw_ncleanse([files_1roundsR, files_5roundsR, files_10roundsR, files_20roundsR], [files_1roundsPR, files_5roundsPR, files_10roundsPR, files_20roundsPR], 'mnist_ncleanse_wm_accuracy', legend=False) 
...
draw_ncleanse([files_1roundsR, files_5roundsR, files_10roundsR, files_20roundsR], 'CIFAR10', 'R', 'cifar_ncleanse_R', legend=False)
...
draw_ncleanse([files_1roundsPR, files_5roundsPR, files_10roundsPR, files_20roundsPR], 'CIFAR10', 'PR', 'cifar_ncleanse_PR')
