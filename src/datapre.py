...
def prepare_data(config: configparser.ConfigParser) -> namedtuple:
    ...
    flipped_watermark_remove=int(config["WATERMARK_REMOVAL"]["flipped_watermark_remove"])
    neural_cleanse_remove=int(config["WATERMARK_REMOVAL"]["neural_cleanse_remove"])
    n_adversaries=int(config["WATERMARK_REMOVAL"]["n_adversaries"])
    after_train_remove=int(config["WATERMARK_REMOVAL"]["after_train_remove"])
    ...
    flipped_watermark_set = flip_watermark(watermark_dataset, watermark_data_save_path, watermark_transforms)
    ...
    watermark_loader = torch.utils.data.DataLoader(watermark_set, batch_size=Batch_size, shuffle = True)

    flipped_watermark_loader = torch.utils.data.DataLoader(flipped_watermark_set, batch_size=Batch_size, shuffle = True)
    ...
    # SETUP WATERMARK REMOVAL PROCEDURE
    RemovalOps = namedtuple("RemovalOps",
                             [
                                 "flipped_watermark_remove",
                                 "neural_cleanse_remove",
                                 "n_adversaries",
                                 "flipped_watermark_set",
                                 "flipped_watermark_loader",
                                 "after_train_remove"
                             ])
    watermark_removal_ops = RemovalOps(
...
def flip_watermark(watermark_dataset_name: str, watermark_data_path: str, transformations: tv.transforms.Compose):
    if watermark_dataset_name == "CPATTERN":
        flipped_watermark_set = dh.ImageNet(watermark_data_path, train=False, transform=transformations, download=True)
    elif watermark_dataset_name == "PATTERN" or "RANDOM" or "IMAGENET" or "MPATTERN":
        flipped_watermark_set = dh.Pattern_flip(watermark_data_path, train=False, transform=transformations, download=True)
    else:
        raise ValueError(watermark_dataset_name)

    log.info("Flipped watermark ({}) samples: {}\nSaved in: {}".format(watermark_dataset_name, len(flipped_watermark_set), watermark_data_path))

    return flipped_watermark_set
